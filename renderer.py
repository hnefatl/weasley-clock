...
class Renderer:
    ...
    def __enter__(self) -> Renderer:
        ...
        self._clock_area = pygame.rect.Rect(0, 0, 1, 1).fit(self._screen.get_rect())
        self._clock_area.center = self._screen.get_rect().center
    ...
    def render(
        ...
        self._render_wheel(zones)
        self._render_hands(people, failures)
        pygame.display.flip()

    def _render_wheel(self, zones: set[str]):
    ...
    def _render_hands(
        ...
        for person, failure in failures.items():
            text = self._font.render(f"{person.name}: {failure}", True, (255, 0, 0))
            self._screen.blit(text, (0, current_height))
            current_height += self._font.get_height()
